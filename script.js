...
let personagemTemp = {
  ...
  // extras (adicionar quantos quiser na ficha)
  extras: {
    oficios: [],
    pericias: [],
    habilidades: [],
...
const IDENTIDADE_INDEX = 4; // sempre liberada
...
function atualizarTopoEtapas() {
  ETAPAS.forEach((nomeEtapa, i) => {
    ...
    const liberada = (i <= etapaLiberadaIndex) || (i === IDENTIDADE_INDEX);

    if (liberada) el.classList.remove("bloqueada");
...
window.irEtapa = function (nomeEtapa) {
  ...
  const podeIr = (idx <= etapaLiberadaIndex) || (idx === IDENTIDADE_INDEX);
  if (!podeIr) return;
...
window.acessarFichaSalva = function (id) {
  ...
  // garantir extras e resDef caso venha ficha antiga
  if (!personagemTemp.extras) {
    personagemTemp.extras = { oficios: [], pericias: [], habilidades: [], poderes: [] };
  }
...
function calcularStatus(atributos) {
  ...
  return {
    HP: 25 + (VIG * 10),             // +2 VIG = 20HP
    MP: 15 + (INT * 20),             // +1 INT = 20MP
    COS: 0 + ((INT + AUR) * 10),     // +1 INT + 1 AUR = 20 COS
    KI: 0 + (FOR * 10),              // +2 FOR = 20KI
    CK: 0 + (AUR * 10)               // +1 AUR = 10CK
  };
}
...
window.abrirAbaFicha = function (aba) {
  ...
  function topoComAdicionar(tituloTexto, tipo) {
    return `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px;">
        <div style="font-weight:bold;">${tituloTexto}</div>
        <button class="btn-mini" onclick="abrirTelaAdicionar('${tipo}')">➕</button>
      </div>
    `;
  }
  ...
  if (aba === "oficios") {
    ...
    conteudo.innerHTML = topoComAdicionar("Adicionar Ofícios", "oficios");

    const lista = [personagemTemp.oficio].filter(Boolean).concat(personagemTemp.extras.oficios || []);
    renderListaComMod(conteudo, "oficios", lista);
    return;
  }

  if (aba === "pericias") {
    titulo.textContent = "Perícias";
    conteudo.innerHTML = topoComAdicionar("Adicionar Perícias", "pericias");

    const lista = (personagemTemp.pericias || []).concat(personagemTemp.extras.pericias || []);
    renderListaComMod(conteudo, "pericias", lista);
    return;
  }

  if (aba === "habilidades") {
    titulo.textContent = "Habilidades";
    conteudo.innerHTML = topoComAdicionar("Adicionar Habilidades", "habilidades");

    const base = (personagemTemp.classe?.habilidades || []).map(h => ({ nome: h, atributo: "Classe" }));
    const extras = (personagemTemp.extras.habilidades || []).map(h => ({ nome: h.nome || h, atributo: h.atributo || "Extra" }));
    renderListaComMod(conteudo, "habilidades", base.concat(extras));
    return;
  }

  if (aba === "poderes") {
    titulo.textContent = "Poderes";
    conteudo.innerHTML = topoComAdicionar("Adicionar Poderes", "poderes");

    const base = (personagemTemp.classe?.oficioPericias || []).map(p => ({ nome: p, atributo: "Classe" }));
    const extras = (personagemTemp.extras.poderes || []).map(p => ({ nome: p.nome || p, atributo: p.atributo || "Extra" }));
    renderListaComMod(conteudo, "poderes", base.concat(extras));
    return;
  }
...
/* =========================
   TELA ADICIONAR (dentro do painel)
========================= */

let telaAdicionarTipo = null;

window.abrirTelaAdicionar = function (tipo) {
  telaAdicionarTipo = tipo;
  ...
  const nomeTitulo = {
    oficios: "Adicionar Ofícios",
    pericias: "Adicionar Perícias",
    habilidades: "Adicionar Habilidades",
    poderes: "Adicionar Poderes"
  }[tipo] || "Adicionar";

  titulo.textContent = nomeTitulo;

  conteudo.innerHTML = `
    <input id="filtroAdicionar" class="filtro" placeholder="filtro" />

    <div id="listaAdicionar" class="lista-cards"></div>

    <button class="btn enabled" style="margin-top:12px;" onclick="voltarAbaOriginal('${tipo}')">
      VOLTAR
    </button>
  `;

  renderTelaAdicionar(tipo, "");

  const filtro = document.getElementById("filtroAdicionar");
  filtro.addEventListener("input", (e) => {
    renderTelaAdicionar(tipo, e.target.value || "");
  });
};

function voltarAbaOriginal(tipo) {
  if (tipo === "oficios") abrirAbaFicha("oficios");
  if (tipo === "pericias") abrirAbaFicha("pericias");
  if (tipo === "habilidades") abrirAbaFicha("habilidades");
  if (tipo === "poderes") abrirAbaFicha("poderes");
}
window.voltarAbaOriginal = voltarAbaOriginal;

function renderTelaAdicionar(tipo, textoFiltro) {
  const container = document.getElementById("listaAdicionar");
  if (!container) return;
  ...
  if (tipo === "oficios") {
    lista = OFICIOS.map(o => ({ nome: o.nome, atributo: o.atributo }));
  }

  if (tipo === "pericias") {
    lista = PERICIAS.map(p => ({ nome: p.nome, atributo: p.atributo }));
  }

  // habilidades/poderes manual
  if (tipo === "habilidades" || tipo === "poderes") {
    container.innerHTML = `
      <div class="card">
        <div class="titulo-card">Adicionar manualmente</div>
        <div class="sub-card">Escreva o nome e adicione quantas quiser</div>

        <div class="campo" style="margin-top:10px;">
          <input id="novoExtraNome" placeholder="Ex: Golpe Rápido" />
        </div>

        <button class="btn enabled" style="margin-top:10px;" onclick="adicionarExtraManual('${tipo}')">
          ADICIONAR
        </button>
      </div>
    `;
  ...
  const filtrado = lista.filter(x => x.nome.toLowerCase().includes(filtro));

  filtrado.forEach((item) => {
    const card = document.createElement("div");
    card.className = "card";

    const jaTem = verificarSeJaTem(tipo, item.nome);

    card.innerHTML = `
      <div class="titulo-card">${item.nome}</div>
      <div class="sub-card">${item.atributo || ""}</div>
      <div class="sub-card" style="margin-top:8px; opacity:0.9;">
        ${jaTem ? "✓ Já adicionado" : "Toque para adicionar"}
      </div>
    `;

    card.onclick = () => {
      if (jaTem) return;
      adicionarExtra(tipo, item);
      card.classList.add("selecionado");
      card.querySelectorAll(".sub-card")[1].innerText = "✓ Já adicionado";
    };
  ...
  if (filtrado.length === 0) {
    container.innerHTML = `
      <div class="card">
        <div class="sub-card">Nada encontrado.</div>
      </div>
    `;
  }
}

function verificarSeJaTem(tipo, nome) {
  if (!personagemTemp.extras) personagemTemp.extras = { oficios: [], pericias: [], habilidades: [], poderes: [] };

  if (tipo === "oficios") {
    const base = personagemTemp.oficio?.nome;
    const extras = (personagemTemp.extras.oficios || []).map(o => o.nome);
    return base === nome || extras.includes(nome);
  }

  if (tipo === "pericias") {
    const base = (personagemTemp.pericias || []).map(p => p.nome);
    const extras = (personagemTemp.extras.pericias || []).map(p => p.nome);
    return base.includes(nome) || extras.includes(nome);
  }

  if (tipo === "habilidades") {
    const base = (personagemTemp.classe?.habilidades || []);
    const extras = (personagemTemp.extras.habilidades || []).map(h => h.nome || h);
    return base.includes(nome) || extras.includes(nome);
  }

  if (tipo === "poderes") {
    const base = (personagemTemp.classe?.oficioPericias || []);
    const extras = (personagemTemp.extras.poderes || []).map(p => p.nome || p);
    return base.includes(nome) || extras.includes(nome);
  }
...
function adicionarExtra(tipo, item) {
  if (!personagemTemp.extras) personagemTemp.extras = { oficios: [], pericias: [], habilidades: [], poderes: [] };

  personagemTemp.extras[tipo] = personagemTemp.extras[tipo] || [];
  personagemTemp.extras[tipo].push(item);

  salvarAutoFicha();
  mostrarToastSalvo();
}

window.adicionarExtraManual = function (tipo) {
  const inp = document.getElementById("novoExtraNome");
  if (!inp) return;

  const nome = inp.value.trim();
  if (!nome) return;

  const item = { nome, atributo: "Extra" };
