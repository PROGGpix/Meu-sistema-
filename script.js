...
let personagemTemp = {
  ...
  // NOVO: lista extra adicionável na ficha
  oficiosExtra: [],
  periciasExtra: [],
  habilidadesExtra: [],
  poderesExtra: [],
...
window.irEtapa = function (nomeEtapa) {
  ...
  if (nomeEtapa !== "identidade" && idx > etapaLiberadaIndex) return;
...
window.acessarFichaSalva = function (id) {
  ...
  personagemTemp.bonusMax = personagemTemp.bonusMax || { HP:0, MP:0, COS:0, KI:0, CK:0 };
  personagemTemp.resdef = personagemTemp.resdef || { RES:0, DEF:0 };

  personagemTemp.oficiosExtra = personagemTemp.oficiosExtra || [];
  personagemTemp.periciasExtra = personagemTemp.periciasExtra || [];
  personagemTemp.habilidadesExtra = personagemTemp.habilidadesExtra || [];
  personagemTemp.poderesExtra = personagemTemp.poderesExtra || [];
...
function calcularStatusBase(atributos) {
...
function garantirRecursos() {
  ...
  personagemTemp.bonusMax = personagemTemp.bonusMax || {
    HP: 0, MP: 0, COS: 0, KI: 0, CK: 0
  };
}
...
function atualizarMaximosPelosAtributos() {
  ...
  const base = calcularStatusBase(personagemTemp.atributos);

  const max = {
    HP: base.HP + Number(personagemTemp.bonusMax.HP || 0),
    MP: base.MP + Number(personagemTemp.bonusMax.MP || 0),
    COS: base.COS + Number(personagemTemp.bonusMax.COS || 0),
    KI: base.KI + Number(personagemTemp.bonusMax.KI || 0),
    CK: base.CK + Number(personagemTemp.bonusMax.CK || 0)
  };

  personagemTemp.recursos.HP.max = Math.max(0, max.HP);
  personagemTemp.recursos.MP.max = Math.max(0, max.MP);
  personagemTemp.recursos.COS.max = Math.max(0, max.COS);
  personagemTemp.recursos.KI.max = Math.max(0, max.KI);
  personagemTemp.recursos.CK.max = Math.max(0, max.CK);
...
function atualizarBarrasFicha() {
  ...
  const resEl = document.getElementById("resValor");
  const defEl = document.getElementById("defValor");
  if (resEl) resEl.value = personagemTemp.resdef?.RES ?? 0;
  if (defEl) defEl.value = personagemTemp.resdef?.DEF ?? 0;
}
...
function ativarEdicaoRecursos() {
  ...
  if (resEl) {
    resEl.oninput = () => {
      personagemTemp.resdef = personagemTemp.resdef || { RES:0, DEF:0 };
      personagemTemp.resdef.RES = Number(resEl.value || 0);
      salvarAutoFicha();
    };
  }
  ...
  if (defEl) {
    defEl.oninput = () => {
      personagemTemp.resdef = personagemTemp.resdef || { RES:0, DEF:0 };
      personagemTemp.resdef.DEF = Number(defEl.value || 0);
      salvarAutoFicha();
    };
  }
}
...
window.alterarMax = function (nome, delta) {
  garantirRecursos();
  ...
  personagemTemp.bonusMax[nome] = Number(personagemTemp.bonusMax[nome] || 0) + delta;

  atualizarMaximosPelosAtributos();

  if (personagemTemp.recursos[nome].atual > personagemTemp.recursos[nome].max) {
    personagemTemp.recursos[nome].atual = personagemTemp.recursos[nome].max;
  }
...
/* =========================
   TELAS DE ADIÇÃO (NOVA)
========================= */

function criarTelaAdicionarSeNaoExistir() {
  if (document.getElementById("tela-add-lista")) return;

  const tela = document.createElement("section");
  tela.id = "tela-add-lista";
  tela.className = "tela";

  tela.innerHTML = `
    <h2 class="titulo" id="addTitulo">Adicionar</h2>
    <input id="addFiltro" class="filtro" placeholder="filtro" />
    <div id="addLista" class="lista-cards"></div>
    <button class="btn enabled" onclick="voltarDaTelaAdicionar()">VOLTAR</button>
  `;

  document.body.appendChild(tela);
}

let addContexto = null;

window.voltarDaTelaAdicionar = function () {
  abrirFichaAtual();
};

function abrirTelaAdicionar(tipo) {
  criarTelaAdicionarSeNaoExistir();

  addContexto = tipo;

  const titulo = document.getElementById("addTitulo");
  const filtro = document.getElementById("addFiltro");
  const lista = document.getElementById("addLista");

  filtro.value = "";

  if (tipo === "oficios") titulo.textContent = "Adicionar Ofícios";
  if (tipo === "pericias") titulo.textContent = "Adicionar Perícias";
  if (tipo === "habilidades") titulo.textContent = "Adicionar Habilidades";
  if (tipo === "poderes") titulo.textContent = "Adicionar Poderes";

  trocarTela("tela-add-lista");
  ...
  function renderCards(dados) {
    ...
    dados.forEach((obj) => {
      ...
      div.innerHTML = `
        <div class="titulo-card">${obj.nome}</div>
        <div class="sub-card">${obj.atributo || ""}</div>
      `;

      div.onclick = () => adicionarNaFicha(tipo, obj);
      lista.appendChild(div);
    });

    if (dados.length === 0) {
      lista.innerHTML = `<div class="card"><div class="sub-card">Nada encontrado.</div></div>`;
    }
  }

  let dadosBase = [];

  if (tipo === "oficios") dadosBase = OFICIOS;
  if (tipo === "pericias") dadosBase = PERICIAS;
  ...
  renderCards(dadosBase);

  filtro.oninput = () => {
    const txt = filtro.value.toLowerCase().trim();
    const filtrado = dadosBase.filter(x => x.nome.toLowerCase().includes(txt));
    renderCards(filtrado);
  };
}

function adicionarNaFicha(tipo, obj) {
  if (tipo === "oficios") {
    personagemTemp.oficiosExtra = personagemTemp.oficiosExtra || [];
    personagemTemp.oficiosExtra.push(obj);
  }

  if (tipo === "pericias") {
    personagemTemp.periciasExtra = personagemTemp.periciasExtra || [];
    personagemTemp.periciasExtra.push(obj);
  }

  if (tipo === "habilidades") {
    personagemTemp.habilidadesExtra = personagemTemp.habilidadesExtra || [];
    personagemTemp.habilidadesExtra.push(obj);
  }

  if (tipo === "poderes") {
    personagemTemp.poderesExtra = personagemTemp.poderesExtra || [];
    personagemTemp.poderesExtra.push(obj);
  }
...
window.abrirAbaFicha = function (aba) {
  ...
  if (aba === "oficios") {
    ...
    conteudo.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px;">
        <div class="sub-card">Ofícios do personagem</div>
        <button class="btn-mini" onclick="abrirTelaAdicionar('oficios')">➕</button>
      </div>
    `;
    const lista = [
      ...(personagemTemp.oficio ? [personagemTemp.oficio] : []),
      ...(personagemTemp.oficiosExtra || [])
    ];
  ...
  if (aba === "pericias") {
    ...
    conteudo.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px;">
        <div class="sub-card">Perícias do personagem</div>
        <button class="btn-mini" onclick="abrirTelaAdicionar('pericias')">➕</button>
      </div>
    `;
    const lista = [
      ...(personagemTemp.pericias || []),
      ...(personagemTemp.periciasExtra || [])
    ];
  ...
  if (aba === "habilidades") {
    ...
    conteudo.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px;">
        <div class="sub-card">Habilidades do personagem</div>
        <button class="btn-mini" onclick="abrirTelaAdicionar('habilidades')">➕</button>
      </div>
    `;
    const base = (personagemTemp.classe?.habilidades || []).map(h => ({ nome: h, atributo: "Classe" }));
    const lista = [
      ...base,
      ...(personagemTemp.habilidadesExtra || [])
    ];
  ...
  if (aba === "poderes") {
    ...
    conteudo.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px;">
        <div class="sub-card">Poderes do personagem</div>
        <button class="btn-mini" onclick="abrirTelaAdicionar('poderes')">➕</button>
      </div>
    `;
    const base = (personagemTemp.classe?.oficioPericias || []).map(p => ({ nome: p, atributo: "Classe" }));
    const lista = [
      ...base,
      ...(personagemTemp.poderesExtra || [])
    ];
